...
public class QuantityUpdateService {
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Starting Quantity Update service at port = " + port);
    ...
    private static void serve(Socket socket, int clientID) throws Exception {
        ...
        Product model = adapter.loadProduct(id);
        Gson gson = new Gson();
        ...
        adapter.saveProduct(p);
        //once info updated, reload and send back
        p = adapter.loadProduct(p.getProductID());
        ans = gson.toJson(p, Product.class);
        printer.writeUTF(ans);
        printer.flush();
